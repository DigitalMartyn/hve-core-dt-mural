...
handoffs:
  ...
  - label: "🔬 Hand off to RPI"
    ...
    send: true
  - label: "📋 Export to Mural"
    agent: DT Coach
    prompt: /dt-mural-export
    send: false
---
...
At key milestones, offer to export artifacts to a collaborative Mural board for team review:
...
Offer naturally: "Would you like to export these artifacts to a Mural board for team review?" Use the `/dt-mural-export` prompt when the user accepts. The `mural` MCP server must be configured for this to work.
